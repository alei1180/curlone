...
Процедура ПрочитатьМетодЗапроса(Команда)
	...
	ЕстьДанныеPOST = ЕстьОпцииГруппыData(Команда) Или ЕстьОпцииГруппыForm(Команда);
...
Процедура ПрочитатьДанныеДляОтправки(Команда)
	...
	ПрочитатьForm(Команда);
	ПрочитатьFormString(Команда);
...
Процедура ПрочитатьForm(Команда)

	МассивДанных = ЗначениеОпции(Команда, "form"); // -f, --Form
...
Процедура ПрочитатьFormString(Команда)

	МассивДанных = ЗначениеОпции(Команда, "form-string");
	Назначение = НазначенияПередаваемыхДанных.ТелоЗапроса;
...
Функция ПолучитьКонсольноеПриложение()
	...
	ДобавитьПоддерживаемуюОпциюВКонсольноеПриложение("F form", "Передаваемые данные по HTTP POST (multipart/form-data)").ТМассивСтрок();
	ДобавитьПоддерживаемуюОпциюВКонсольноеПриложение("form-string", "Передаваемые данные по HTTP POST без интерпретации спецтальных сиволов (multipart/form-data)").ТМассивСтрок();
...
Процедура ПроверитьДанные(Команда)
	...
	Если ЕстьОпцииГруппыData(Команда) И ЕстьОпцииГруппыForm(Команда) Тогда
		ТекстОшибки = СтрШаблон(ШаблонОдновременнаяПередача, "-d (--data)", "-F (--form)");
		ИсходящиеОшибки.Добавить(ОбщегоНазначения.НоваяКритичнаяОшибка(ТекстОшибки));
		Возврат;
	КонецЕсли;

	Если ЕстьОпцииГруппыForm(Команда) И ЗначениеОпции(Команда, "head") = Истина Тогда
		ТекстОшибки = СтрШаблон(ШаблонОдновременнаяПередача, "-I (--head)", "-F (--form)");
		ИсходящиеОшибки.Добавить(ОбщегоНазначения.НоваяКритичнаяОшибка(ТекстОшибки));
		Возврат;
	КонецЕсли;

	Если ЕстьОпции(Команда, "upload-file") И ЕстьОпцииГруппыForm(Команда) Тогда
...
Функция ЕстьОпцииГруппыData(Команда)
	Возврат ЕстьОпции(Команда, "d,data,data-raw,data-binary,data-urlencode,data-ascii,json");
КонецФункции

Функция ЕстьОпцииГруппыForm(Команда)
	Возврат ЕстьОпции(Команда, "F,form,form-string");
КонецФункции
