...
Перем КонсольноеПриложение; // КонсольноеПриложение
Перем ТекущаяКоманда; // КомандаПриложения
...
Перем АргументыТекущейКоманды; // Массив из Строка
...
Функция Конвертировать(КоманднаяСтрока, ГенераторПрограммногоКода = Неопределено, Ошибки = Неопределено) Экспорт
	...
	Иначе
		...
		РезультатПарсинга = Парсер.Распарсить(КоманднаяСтрока);
		АргументыКоманд = РезультатПарсинга.АргументыКоманд;
		ИменаПеременных = РезультатПарсинга.ИменаПеременных;
	КонецЕсли;

	Если АргументыКоманд.Количество() = 0 Тогда
		Текст = "Передана пустая команда";
		ИсходящиеОшибки.Добавить(ОбщийНаборИнструментов.НоваяКритичнаяОшибка(Текст));
	...
	Для Каждого АргументыКоманды Из АргументыКоманд Цикл
		...
		ОписаниеЗапроса.ИменаПеременных = ИменаПеременных;
		...
		СгенерированныйКод = "";
		Если Не ОбщийНаборИнструментов.ЕстьКритичныеОшибки(ИсходящиеОшибки) Тогда
	...
	Для Каждого ОписаниеРесурса Из ОписаниеЗапроса.АдресаРесурсов Цикл
		...
		Если Не ПустаяСтрока(ОшибочныеМетоды) Тогда
			...
			ТекстОшибки = СтрШаблон(
				"Запрещено одновременное использование нескольких HTTP методов %1 для URL %2", 
				ОшибочныеМетоды,
				ОписаниеРесурса.URL
			);
...
Процедура ПрочитатьАдресаРесурсов()
	...
	ОбщийНаборИнструментов.ДополнитьМассив(МассивURL, ТекущаяКоманда.ЗначениеАргумента("URL"));
	ОбщийНаборИнструментов.ДополнитьМассив(МассивURL, ЗначениеОпции("url"));
...
Процедура ПрочитатьИменаВыходныхФайлов()
	...
	ИзвлекатьДляВсехИмяФайлаИзURL = УстановленФлагОпции("remote-name-all");
	...
	Для Каждого ИмяВыходногоФайла Из ИменаВыходныхФайлов Цикл
		...
		Если ЗначениеЗаполнено(КаталогСохраненияФайлов) Тогда
			ИмяВыходногоФайла = 
				ОбщийНаборИнструментов.ДобавитьКонечныйРазделительПути(КаталогСохраненияФайлов) 
				+ ИмяВыходногоФайла;
		КонецЕсли;
	...
	Для Индекс = 0 По ОписаниеЗапроса.АдресаРесурсов.ВГраница() Цикл
		...
		ИзвлекатьИмяИзURL = Индекс <= ПризнакиИзвлеченияИмениФайлаИзURL.ВГраница()
			...
			Или ИзвлекатьДляВсехИмяФайлаИзURL;
		...
		РазборURL = Новый РазборURL(ОписаниеАдреса.URL);
		
		ИндексСлеша = СтрНайти(РазборURL.Путь, "/", НаправлениеПоиска.СКонца);
		Если ИндексСлеша Тогда
			ИмяВыходногоФайла = СокрЛП(Сред(РазборURL.Путь, ИндексСлеша + 1));
		Иначе
			ИмяВыходногоФайла = РазборURL.Путь;
		КонецЕсли;
...
Функция РазобратьЗаголовки()
	...
	Для Каждого Строка Из МассивЗаголовков Цикл
		...
		Если Лев(Строка, 1) = "@" Тогда
			ТекстОшибки = СтрШаблон(
				"Чтение заголовков из файла для опции -H, --header не поддерживается: %1",
				Сред(Строка, 2)
			);
...
Процедура ПрочитатьТипАутентификации()
	...
	МножественноеИспользование = ЕстьBasic
		+ ЕстьDigest
		+ ЕстьNTLM
		+ ЕстьNegotiate
		+ ЗначениеЗаполнено(AWS4)
		+ ЗначениеЗаполнено(ТокенBearer);
...
Процедура ПрочитатьПользователя()
	...
	ОписаниеЗапроса.ПереданыУчетныеДанныеПользователя = Истина;

	ИндексДвоеточия = СтрНайти(ПользовательИПароль, ":");
	Если ИндексДвоеточия > 0 Тогда
		ИмяПользователя = Лев(ПользовательИПароль, ИндексДвоеточия - 1);
		ПарольПользователя = Сред(ПользовательИПароль, ИндексДвоеточия + 1);
	Иначе
		ИмяПользователя = ПользовательИПароль;
		ПарольПользователя = "";
	КонецЕсли;
...
Процедура ПрочитатьData()
	...
	ПорядкиДанных = ПорядкиЗначенийОпции("d data data-ascii");
	...
	Для Каждого Данные Из МассивДанных Цикл
		...
		Если Лев(Данные, 1) = "@" Тогда
			...
			УстановитьПорядокДанных(ПередаваемыйФайл, ПорядкиДанных, НомерДанных);
		...
		Иначе
			...
			Для Каждого ДанныеПоля Из МассивДанныхПолей Цикл
				...
				УстановитьПорядокДанных(ПередаваемыйТекст, ПорядкиДанных, НомерДанных);
...
Процедура ПрочитатьDataRaw()
	...
	ПорядкиДанных = ПорядкиЗначенийОпции("data-raw");
	Назначение = НазначениеПередаваемыхДанных();
	НомерДанных = 0;
...
Процедура ПрочитатьDataBinary()
	...
	ПорядкиДанных = ПорядкиЗначенийОпции("data-binary");
	Назначение = НазначениеПередаваемыхДанных();
	НомерДанных = 0;
	...
	Для Каждого Данные Из МассивДанных Цикл	
		...
		Если Лев(Данные, 1) = "@" Тогда
			...
			ПередаваемыйФайл = Новый ПередаваемыйФайл(ИмяФайла, Назначение);
			ПередаваемыйФайл.ПрочитатьСодержимое = Истина;
			ПередаваемыйФайл.ДвоичноеСодержимое = Истина;
			УстановитьПорядокДанных(ПередаваемыйФайл, ПорядкиДанных, НомерДанных);
			ОписаниеЗапроса.Файлы.Добавить(ПередаваемыйФайл);
...
Процедура ПрочитатьDataUrlencode()
	...
	ПорядкиДанных = ПорядкиЗначенийОпции("data-urlencode");
	Назначение = НазначениеПередаваемыхДанных();
	НомерДанных = 0;
...
Процедура ПрочитатьОпциюJson()

	МассивДанных = ЗначениеОпции("json");
	ПорядкиДанных = ПорядкиЗначенийОпции("json");
	Назначение = НазначениеПередаваемыхДанных();
	НомерДанных = 0;
...
Процедура ОбработатьДанныеПоляФормы(Данные)
	...
	Для Каждого ОписаниеЗначения Из ДанныеПоляФормы.Значения Цикл
		...
		Для Каждого ПутьКФайлуЗаголовков Из ОписаниеЗначения.ФайлыЗаголовков Цикл
			ТекстОшибки = СтрШаблон(
				"Чтение заголовков multipart-части из файла для опции -F, --form не поддерживается: %1",
...
Процедура ПрочитатьАдресПрокси()
	...
	РазборURL = Новый РазборURL(АдресПрокси);

	ОписаниеЗапроса.ПроксиСервер = РазборURL.Сервер;
	ОписаниеЗапроса.ПроксиПользователь = РазборURL.Аутентификация.Пользователь;
	ОписаниеЗапроса.ПроксиПароль = РазборURL.Аутентификация.Пароль;
	
	ОписаниеЗапроса.ПроксиПротокол = НРег(РазборURL.Схема);
	Если Не ЗначениеЗаполнено(ОписаниеЗапроса.ПроксиПротокол) Тогда
		ОписаниеЗапроса.ПроксиПротокол = ПротоколыURL.HTTP;
	КонецЕсли;

	ОписаниеЗапроса.ПроксиПорт = РазборURL.Порт;
...
Функция ПолучитьКонсольноеПриложение()
	...
	ДобавитьПоддерживаемуюОпцию(
		"form-string", 
		"Передаваемые данные по HTTP POST без интерпретации специальных символов (multipart/form-data)").ТМассивСтрок();
...
Процедура ОбработатьАргументыКоманды(АргументыКоманды, НомерКоманды, КоличествоКоманд)
	...
	АргументыТекущейКоманды = Новый Массив();
	ОбщийНаборИнструментов.ДополнитьМассив(АргументыТекущейКоманды, АргументыКоманды);
...
Функция ПорядкиЗначенийОпции(Наименования)

	Результат = Новый Массив();
	Синонимы = СтрРазделить(Наименования, " ", Ложь);

	Для Индекс = 0 По АргументыТекущейКоманды.ВГраница() Цикл
		Аргумент = АргументыТекущейКоманды[Индекс];

		Для Каждого Синоним Из Синонимы Цикл
			Префикс = ?(СтрДлина(Синоним) = 1, "-", "--");
			Наименование = Префикс + Синоним;
			ЭтоОтдельнаяОпция = Аргумент = Наименование;
			ЭтоДлиннаяОпцияСоЗначением = СтрДлина(Синоним) > 1
				И СтрНачинаетсяС(Аргумент, Наименование + "=");
			ЭтоКороткаяОпцияСоЗначением = СтрДлина(Синоним) = 1
				И СтрНачинаетсяС(Аргумент, Наименование)
				И СтрДлина(Аргумент) > СтрДлина(Наименование);

			Если ЭтоОтдельнаяОпция Или ЭтоДлиннаяОпцияСоЗначением
				Или ЭтоКороткаяОпцияСоЗначением Тогда
...
Процедура УстановитьПорядокДанных(ПередаваемыеДанные, ПорядкиДанных, НомерДанных)

	Если НомерДанных <= ПорядкиДанных.ВГраница() Тогда
		ПередаваемыеДанные.Порядок = ПорядкиДанных[НомерДанных];
	КонецЕсли;
...
Процедура ПроверитьНаличиеURL(ОписаниеЗапроса)
	...
	Если ОписаниеЗапроса.АдресаРесурсов.Количество() = 0 Тогда
		ИсходящиеОшибки.Добавить(ОбщийНаборИнструментов.НоваяКритичнаяОшибка("Не указан URL"));
		Возврат;
	КонецЕсли;
...
Процедура ВывестиРазделительКоманд(Результат, НомерКоманды, КоличествоКоманд)
	...
	Результат = Результат
		...
		+ СтрШаблон(
			"//////////////////////////////////////////////
			|// %1 #%2.", ПакетРесурсов.ПолучитьСтроку("Текст.Команда"), 
			НомерКоманды
		);
...
Функция MIMEТипПоРасширениюФайла(Расширение)
	...
	Если MIMEТипыРасширенийФайлов = Неопределено Тогда
		...
		MIMEТипыРасширенийФайлов.Вставить(".html", "text/html");
...
Процедура Заглушка(Значение = Неопределено) Экспорт // BSLLS:MissingParameterDescription-off
