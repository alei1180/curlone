...
Перем ИмяПараметраЧастиДанных; // Строка
...
//
// Параметры:
//   Описание - ОписаниеЗапроса - Описание запроса
...
Процедура ПриСозданииОбъекта()
	...
	ИмяПараметраЧастиДанных = "ЧастиДанных";
...
Процедура ДобавитьДанныеЗапроса()
	...
	Если ОписаниеЗапроса.ДвоичныеФайлыПоНазначению(НазначениеДанных).Количество() > 0 Тогда
		Код = Новый ГенераторКодаДвоичныеДанныеЗапроса(
			ОписаниеЗапроса,
			НазначениеДанных,
			ПрочитанныеФайлы,
			ИмяПараметраДанныеЗапроса,
			ИмяПараметраЧастиДанных
		).Получить();
	...
	Если ВозможноПередатьОтправляемыеДанныеЧерезСоответствие(НазначениеДанных) Тогда
	
		ДобавитьТелоЗапросаЧерезСоответствие(ИмяПараметраДанныеЗапроса, ДанныеЗапросаСборка);
...
Процедура ДобавитьПараметрыЗапроса()
	...
	Если Не (ЕстьПараметрыЗапроса() И ВозможноПередатьПараметрыЗапросаЧерезСоответствие()) Тогда
		Возврат;
	КонецЕсли;

	ПараметрыЗапроса = Новый Соответствие();
	Назначение = НазначенияПередаваемыхДанных.СтрокаЗапроса;
	
	ПолучитьПараметрыЗапросаИзURL(ПараметрыЗапроса);
	ПолучитьПараметрыИзТекстовыхДанных(Назначение, ПараметрыЗапроса);
	ПолучитьПараметрыИзПрочитанныхФайлов(Назначение, ПараметрыЗапроса);
	
	Если ПараметрыЗапроса.Количество() > 0 Тогда

		Конструктор
			.ДобавитьПустуюСтроку()
			.ДобавитьСоответствие(ПараметрыЗапроса, ИмяПараметраПараметрыЗапроса);

	КонецЕсли;

КонецПроцедуры

Процедура ДобавитьТелоЗапросаЧерезСоответствие(ИмяПараметра, РезультатСборка)

	Назначение = НазначенияПередаваемыхДанных.ТелоЗапроса;
	ДанныеТела = Новый Соответствие();

	ПолучитьПараметрыИзТекстовыхДанных(Назначение, ДанныеТела);
	ПолучитьПараметрыИзПрочитанныхФайлов(Назначение, ДанныеТела);

	Если ДанныеТела.Количество() > 0 Тогда

		Конструктор
			.ДобавитьПустуюСтроку()
			.ДобавитьСоответствие(ДанныеТела, ИмяПараметра);
...
Процедура ПолучитьПараметрыЗапросаИзURL(ИсходящиеПараметры)

	Если ОписаниеЗапроса.АдресаРесурсов.Количество() > 1 Тогда
		Возврат;
	КонецЕсли;

	ОписаниеРесурса = ОписаниеЗапроса.АдресаРесурсов[0];
	СтруктураURL = Новый РазборURL(ОписаниеРесурса.URL);

	Для Каждого Параметр Из СтруктураURL.ПараметрыЗапроса Цикл
		ОбщийНаборИнструментов.ВставитьВСловарьСписков(
			ИсходящиеПараметры,
			Параметр.Ключ,
			РаскодироватьЗначениеПередаваемыхДанных(Параметр.Значение)
		);
	КонецЦикла;

КонецПроцедуры

Функция РаскодироватьЗначениеПередаваемыхДанных(Значение)

	Если ТипЗнч(Значение) = Тип("Массив") Тогда

		Результат = Новый Массив();
		Для Каждого Элемент Из Значение Цикл
			Результат.Добавить(РаскодироватьЗначениеПередаваемыхДанных(Элемент));
		КонецЦикла;
	...
	Возврат РаскодироватьСтроку(Значение, СпособКодированияСтроки.URLВКодировкеURL);

КонецФункции

Процедура ПолучитьПараметрыИзТекстовыхДанных(Назначение, ИсходящиеПараметры)
	...
	Для Каждого ПередаваемыйТекст Из ОписаниеЗапроса.ОтправляемыеТекстовыеДанные Цикл
		...
		Если Не ПередаваемыйТекст.КодироватьЗначение Тогда
			Значение = РаскодироватьЗначениеПередаваемыхДанных(Значение);
		КонецЕсли;

		ОбщийНаборИнструментов.ВставитьВСловарьСписков(ИсходящиеПараметры, Ключ, Значение);
		
	КонецЦикла;

КонецПроцедуры

Процедура ПолучитьПараметрыИзПрочитанныхФайлов(Назначение, ИсходящиеПараметры)
	...
	Для Каждого ПрочитанныйФайл Из ПрочитанныеФайлы Цикл
		...
		Если Не ПередаваемыйФайл.Назначение = Назначение 
			Или Не ПередаваемыйФайл.ПрочитатьСодержимое
			Или Не ЗначениеЗаполнено(ПередаваемыйФайл.ИмяПоля) Тогда
			Продолжить;
		КонецЕсли;
		
		ОбщийНаборИнструментов.ВставитьВСловарьСписков(ИсходящиеПараметры, ПередаваемыйФайл.ИмяПоля, ПрочитанныйФайл.ИмяПеременной);
	...
	Если СтрНайти(ТипКонтента, "application/json") Тогда
		...
		Если Не Результат = Неопределено Тогда
			...
			Если ТипЗнч(Результат) = Тип("Массив") Тогда
				Конструктор.ДобавитьМассив(Результат, ИмяПараметра);
			Иначе
				Конструктор.ДобавитьСоответствие(Результат, ИмяПараметра);
			КонецЕсли;
...
Процедура ДобавитьВызовМетода(ОписаниеРесурса, Знач СтруктураURL = Неопределено, Знач ДанныеЗапроса = Неопределено)
	...
	Если ОписаниеРесурса.Метод = "GET" Тогда
		...
		Если ЕстьПараметрыЗапроса() И ВозможноПередатьПараметрыЗапросаЧерезСоответствие() Тогда
...
Процедура ДобавитьURL(СтруктураURL)
	...
	Если ОписаниеЗапроса.ПереданыУчетныеДанныеПользователя Тогда
		СтруктураURL.Аутентификация.Пользователь = "";
		СтруктураURL.Аутентификация.Пароль = "";
	КонецЕсли;

	Если ВозможноПередатьПараметрыЗапросаЧерезСоответствие() Тогда

		СтруктураURL.ПараметрыЗапроса.Очистить();
		
		ГенераторКодаURL = Новый ГенераторКодаURL(
			ОписаниеЗапроса, 
			СтруктураURL
		);
	...
	URLСборка = ГенераторКодаURL.Получить();
...
Процедура ДобавитьТаймаутВДополнительныеПараметры(КонструкторДопПараметров)
	...
	Таймаут = ОписаниеЗапроса.Таймаут;
...
Процедура ДобавитьПараметрыЗапросаВДополнительныеПараметры(КонструкторДопПараметров, ОписаниеРесурса)
	...
	Если ЕстьПараметрыЗапроса() 
		И ВозможноПередатьПараметрыЗапросаЧерезСоответствие()
...
Функция ЕстьПараметрыЗапроса()

	АдресаРесурсов = ОписаниеЗапроса.АдресаРесурсов;
	ЕстьПараметрыЗапросаВАдресе = ОписаниеЗапроса.АдресаРесурсов.Количество() = 1 
		И СтрНайти(АдресаРесурсов[0].URL, "?");

	Возврат Состояние.ПереданаСтрокаЗапроса Или ЕстьПараметрыЗапросаВАдресе;

КонецФункции

Функция ВозможноПередатьПараметрыЗапросаЧерезСоответствие()

	Возврат ВозможноПередатьПараметрыЗапросаИзОтправляемыхДанныхЧерезСоответствие()
		И ВозможноПередатьПараметрыЗапросаИзURLЧерезСоответствие();

КонецФункции

Функция ВозможноПередатьПараметрыЗапросаИзURLЧерезСоответствие()
	
	АдресаРесурсов = ОписаниеЗапроса.АдресаРесурсов;

	Если АдресаРесурсов.Количество() <> 1 Тогда
		Возврат Ложь;
	КонецЕсли;

	Если ОписаниеЗапроса.ИменаПеременных.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;

	ПрефиксХешаПеременной = ОбщийНаборИнструментов.ПрефиксХешаПеременной();
	СтруктураURL = Новый РазборURL(АдресаРесурсов[0].URL);

	Для Каждого ПараметрЗапроса Из СтруктураURL.ПараметрыЗапроса Цикл
		Если Не ЗначениеЗаполнено(ПараметрЗапроса.Значение) 
			И СтрНайти(ПараметрЗапроса.Ключ, ПрефиксХешаПеременной) > 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ВозможноПередатьПараметрыЗапросаИзОтправляемыхДанныхЧерезСоответствие()

	Возврат Не Состояние.ПереданаСтрокаЗапроса
		Или ВозможноПередатьОтправляемыеДанныеЧерезСоответствие(НазначенияПередаваемыхДанных.СтрокаЗапроса);

КонецФункции

Функция ВозможноПередатьОтправляемыеДанныеЧерезСоответствие(Назначение)
