...
Процедура ДобавитьТекстовыеДанныеЗапросаЧерезСтроку(НазначениеДанных, ИмяПараметра, РезультатСборка)
	...
	Сборка = ЛокальныйГенераторКода.ДанныеЗапроса(
		НазначениеДанных,
...
Процедура ДобавитьПоследовательнуюОтправкуДвоичныхДанныхРесурса(ОписаниеРесурса)
	...
	ОбщийНаборИнструментов.ДополнитьМассив(ВсеФайлы, ОписаниеЗапроса.Файлы);
	ОбщийНаборИнструментов.ДополнитьМассив(ВсеФайлы, ОписаниеРесурса.Файлы);
...
Процедура ДобавитьURL(СтруктураURL)
	...
	Если Состояние.ПереданаСтрокаЗапроса
		...
		URLСборка = ЛокальныйГенераторКода.URL(СтруктураURL);

	Иначе

		URLСборка = ЛокальныйГенераторКода.URL(
			СтруктураURL,
			ОписаниеЗапроса.ОтправляемыеТекстовыеДанные, 
			ПрочитанныеФайлы
		);
