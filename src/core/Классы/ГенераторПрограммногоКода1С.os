...
Функция Получить(пОписаниеЗапроса) Экспорт
	...
	Для Каждого ОписаниеРесурса Из ОписаниеЗапроса.АдресаРесурсов Цикл
		...
		Если ЭтоHTTP(СтруктураURL.Схема) Тогда
		...
		ИначеЕсли ЭтоFTP(СтруктураURL.Схема) Тогда
			ВывестиFTPСоединение(СтруктураURL);
			ВывестиВызовFTPМетода(СтруктураURL, ОписаниеРесурса.ИмяВыходногоФайла);
		Иначе
...
Функция ПолучитьПорт(СтруктураURL)
	...
	Если Не ЗначениеЗаполнено(Порт) Тогда
		Если СтруктураURL.Схема = "https" Тогда
			Порт = 443;
		ИначеЕсли СтруктураURL.Схема = "http" Тогда
			Порт = 80;
		ИначеЕсли СтруктураURL.Схема = "ftps" Тогда
			Порт = 990;
		ИначеЕсли СтруктураURL.Схема = "ftp" Тогда
			Порт = 21;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиВызовFTPМетода(СтруктураURL, ИмяВыходногоФайла)

	ВывестиПустуюСтроку();

	ПараметрыФункции = Новый Массив;
	ПараметрыФункции.Добавить(ОбернутьКавычками(СтруктураURL.Путь));
	ПараметрыФункции.Добавить(ОбернутьКавычками(?(ЗначениеЗаполнено(ИмяВыходногоФайла), ИмяВыходногоФайла, "path/to/file")));
...
Функция ИспользуетсяЗащищенноеСоединение(СтруктураURL)
	Возврат СтруктураURL.Схема = "https" 
		Или СтруктураURL.Схема = "ftps"
		Или ЗначениеЗаполнено(ОписаниеЗапроса.ИмяФайлаСертификатаКлиента);
КонецФункции

Функция ЭтоHTTP(Схема)
	Возврат Схема = "http" Или Схема = "https";
КонецФункции

Функция ЭтоFTP(Схема)
	Возврат Схема = "ftp" Или Схема = "ftps";
КонецФункции
