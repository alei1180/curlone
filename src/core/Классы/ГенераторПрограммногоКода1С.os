...
Перем ИмяПараметраЧастиТелаЗапроса; // Строка
...
Процедура ПриСозданииОбъекта()
	...
	ИмяПараметраЧастиТелаЗапроса = "ЧастиТелаЗапроса";
...
Процедура ДобавитьЗаголовки()
	...
	ЛокальныйКонструктор = Новый КонструкторПрограммногоКода(ОписаниеЗапроса.ИменаПеременных);
...
Процедура ДобавитьДанныеЗапроса()
	...
	ТелоЗапросаСтрока = "";
	ТелоЗапроса = Новый ГенераторКодаДанныеЗапроса(
		ОписаниеЗапроса, 
		НазначенияПередаваемыхДанных.ТелоЗапроса, 
		ПрочитанныеФайлы
	).Получить();
...
Процедура ДобавитьHTTPСоединение(СтруктураURL)
	...
	ПараметрыАутентификации = ПараметрыАутентификацииВСтроку(СтруктураURL);
	...
	ПараметрыФункции.Добавить(ПараметрыАутентификации.ИмяПользователя);
	ПараметрыФункции.Добавить(ПараметрыАутентификации.ПарольПользователя);
...
Функция ПараметрыАутентификацииВСтроку(СтруктураURL)

	ИмяПользователя = ОписаниеЗапроса.ИмяПользователя;
	ПарольПользователя = ОписаниеЗапроса.ПарольПользователя;

	Если Не ОписаниеЗапроса.ПереданыУчетныеДанныеПользователя Тогда
		ИмяПользователя = СтруктураURL.Аутентификация.Пользователь;
		ПарольПользователя = СтруктураURL.Аутентификация.Пароль;
	КонецЕсли;

	ПередаватьОбязательныеПараметры = ОписаниеЗапроса.ПереданыУчетныеДанныеПользователя
		И Не ОписаниеЗапроса.ТипАутентификации = ТипыАутентификации.AWS4;

	Если ПередаватьОбязательныеПараметры Тогда
		ИмяПользователя = Конструктор.ПараметрВСтроку(ИмяПользователя);
		ПарольПользователя = Конструктор.ПараметрВСтроку(ПарольПользователя);
	Иначе
		ИмяПользователя = Конструктор.НеобязательныйПараметрВСтроку(ИмяПользователя);
		ПарольПользователя = Конструктор.НеобязательныйПараметрВСтроку(ПарольПользователя);
	КонецЕсли;
...
Процедура ДобавитьHTTPЗапрос(СтруктураURL)
	...
	АдресРесурсаКод = Новый ГенераторКодаАдресРесурса(
		ОписаниеЗапроса,
		СтруктураURL, 
...
Процедура ДобавитьУстановкуТелаЗапроса(ОписаниеРесурса)
	...
	ДобавитьУстановкуТелаЗапросаДвоичнымиДанными();
	ДобавитьУстановкуТелаЗапросаИзФайла(ОписаниеРесурса);
	ДобавитьЗаписьДанныхВПотокMultipart();

КонецПроцедуры

Процедура ДобавитьУстановкуТелаЗапросаДвоичнымиДанными()

	Если Не МетодУстановкиТелаЗапроса = "ДвоичныеДанные" Тогда
		Возврат;
	КонецЕсли;

	Конструктор.ДобавитьСтроку(
		"%1.УстановитьТелоИзДвоичныхДанных(%2);",
		ИмяПараметраHTTPЗапрос,
		ИмяПараметраТелоЗапросаСтрока
	);
...
Процедура ДобавитьУстановкуТелаЗапросаИзФайла(ОписаниеРесурса)
	...
	Файлы = Новый Массив();
	КоллекцииМоделиCURL.Дополнить(Файлы, ОписаниеЗапроса.Файлы);
	КоллекцииМоделиCURL.Дополнить(Файлы, ОписаниеРесурса.Файлы);
...
Процедура ДобавитьЗаписьДанныхВПотокMultipart()
	...
	КонструкторЗаписиДанных = Новый КонструкторПрограммногоКода(ОписаниеЗапроса.ИменаПеременных);
...
Процедура ВывестиВызовОтправкиФайлаFTP(ОписаниеРесурса, СтруктураURL)
	...
	Для Каждого ПередаваемыйФайл Из ОписаниеРесурса.Файлы Цикл
		...
		Если ПередаваемыйФайл.ДобавлятьИмяФайлаКURL Тогда
			АдресРесурса = ОбщийНаборИнструментов.ДополнитьИменемФайлаПутьURL(
				ПередаваемыйФайл.ПолноеИмяФайла,
				СтруктураURL.Путь
			);
		Иначе
...
Процедура ОпределитьМетодУстановкиТелаЗапроса()
	...
	КоличествоДвоичныхФайлов = ОписаниеЗапроса.ДвоичныеФайлыПоНазначению(
		НазначенияПередаваемыхДанных.ТелоЗапроса
	).Количество();
	...
	ИначеЕсли КоличествоДвоичныхФайлов > 0
		И (КоличествоФайлов > 1 Или ЕстьТекстовоеТелоЗапроса) Тогда
		МетодУстановкиТелаЗапроса = "ДвоичныеДанные";
