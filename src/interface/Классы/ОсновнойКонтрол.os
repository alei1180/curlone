...
// Конвертирует команду curl из query-параметров GET-запроса или JSON-тела POST-запроса.
//
// Параметры:
//   Запрос - ВходящийЗапрос - HTTP-запрос к точке маршрута.
//   Ответ - Ответ - формируемый HTTP-ответ.
Процедура Конвертировать(Запрос, Ответ) Экспорт
	Перем Ошибки;
	...
	Если Запрос.Метод = "POST" Тогда
		ПараметрыЗапроса = Запрос.ТелоЗапросаОбъект(Тип("Соответствие"));
	КонецЕсли;
	
	ТекстКоманды = ПараметрыЗапроса.Получить("cmd");
	Язык = ПараметрыЗапроса.Получить("lang");
	Локаль = ПараметрыЗапроса.Получить("locale");
	ИмяПараметраТипаОтвета = ?(Запрос.Метод = "POST", "responseType", "response-type");
	ДесериализоватьОтветИзJSON = НРег(ПараметрыЗапроса.Получить(ИмяПараметраТипаОтвета)) = "json";
