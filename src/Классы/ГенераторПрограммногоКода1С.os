...
Процедура ВывестиИнициализациюОтправляемыхТекстовыхДанныхИзФайла()
	...
	НомерФайла = 1;
	Для Каждого ОписаниеФайла Из ОписаниеЗапроса.Файлы Цикл

		Если Не (ОписаниеФайла.ПрочитатьСодержимое 
		...
		Если ОписаниеФайла.УдалятьПереводыСтрок Тогда
			Шаблон = Шаблон + "
			|%1 = СтрЗаменить(%1, Символы.ПС, """");
			|%1 = СтрЗаменить(%1, Символы.ВК, """");";
		КонецЕсли;

		Если ОписаниеФайла.КодироватьСодержимое Тогда
			Шаблон = Шаблон + "
			|%1 = КодироватьСтроку(%1, СпособКодированияСтроки.URLВКодировкеURL);";
		КонецЕсли;

		Если ЗначениеЗаполнено(ОписаниеФайла.Ключ) Тогда
			Шаблон = Шаблон + "
			|%1 = """ + ОписаниеФайла.Ключ + "="" + %1;";
		КонецЕсли;

		Текст = СтрШаблон(Шаблон, 
			ИмяПараметра, 
			ОбернутьКавычками(ОписаниеФайла.ИмяФайла));
...
Процедура ВывестиУстановкуТелаЗапросаДвоичнымиДанными()
	...
	Для Каждого ОписаниеФайла Из ОписаниеЗапроса.Файлы Цикл
		...
		Если ОписаниеФайла.ПрочитатьСодержимое Тогда
			Продолжить;
		КонецЕсли;
