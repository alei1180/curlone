...
// Выполняет конвертацию команды curl в программный код и выводит результат в консоль.
//
// Исключения:
//   argument.invalid_value - Команда convert не найдена во входящих аргументах.
Процедура Конвертировать() Экспорт
	...
	Если Не НайденаКоманда() Тогда
		ВызватьИсключение СтандартныеОшибки.НедопустимоеЗначениеАргумента(
			"Аргументы",
			"Первый аргумент должен содержать команду convert"
		);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(РезультатКонвертации) Тогда
		...
		Консоль.Вывести(РезультатКонвертации + Символы.ПС);

	КонецЕсли;

КонецПроцедуры

// Проверяет наличие команды convert в аргументах.
//
