...
Функция ПостроитьСпецификацию()
	...
	ДобавитьОпределения(Состояние, ПоддерживаемыеОпределения());
	ДобавитьОпределения(Состояние, НеподдерживаемыеОпределения());
...
Процедура ДобавитьОпределения(Состояние, Данные)
	...
	Для Каждого СтрокаОпределения Из Строки Цикл
		...
		ДобавитьОписание(Состояние, Псевдонимы, КодТипа);
	КонецЦикла;

КонецПроцедуры

Процедура ДобавитьОписание(Состояние, ИсходныеПсевдонимы, КодТипа)
	...
	Если ДопускаетОтрицательнуюФорму И СтрНачинаетсяС(КаноническоеИмя, "no-") Тогда
		КоллекцииМоделиCURL.Дополнить(Псевдонимы, ИсходныеПсевдонимы);
		КоллекцииМоделиCURL.Дополнить(ОтрицательныеПсевдонимы, ИсходныеПсевдонимы);
	...
	Описание = Новый ОписаниеОпцииCURL(
		...
		ИдентификаторОбработчика(КаноническоеИмя),
		Ложь
	);
...
Функция ИдентификаторОбработчика(Имя)

	Если СтрНайти(",url,output,output-dir,create-dirs,remote-name,remote-name-all,upload-file,", "," + Имя + ",") > 0 Тогда
		Возврат "resource_and_result";
	КонецЕсли;

	Если СтрНайти(",request,get,head,list-only,header,user-agent,url-query,", "," + Имя + ",") > 0 Тогда
		Возврат "request_and_headers";
	КонецЕсли;

	Если СтрНайти(",data,data-ascii,data-raw,data-binary,data-urlencode,json,form,form-string,", "," + Имя + ",") > 0 Тогда
		Возврат "body";
	КонецЕсли;

	Если СтрНайти(",user,basic,digest,ntlm,negotiate,oauth2-bearer,aws-sigv4,", "," + Имя + ",") > 0 Тогда
		Возврат "authentication";
	КонецЕсли;

	Если СтрНайти(",cert,ca-native,cacert,", "," + Имя + ",") > 0 Тогда
		Возврат "tls";
	КонецЕсли;

	Если СтрНайти(",proxy,proxy-user,proxy-basic,proxy-ntlm,", "," + Имя + ",") > 0 Тогда
		Возврат "proxy";
	КонецЕсли;

	Если СтрНайти(",max-time,connect-timeout,ftp-pasv,ftp-port,location,retry,retry-max-time,", "," + Имя + ",") > 0 Тогда
