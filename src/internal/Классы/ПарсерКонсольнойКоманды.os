...
Функция ПолучитьСледующийТокен()
	...
	СохранитьОбратныйСлешПриНеобходимости(Токен);
	...
	Если Токен.ТипТокена() = "СимволЭкранирования" Тогда
		...
		ИначеЕсли Не ОткрытаКавычкаБезПоддержкиИнтерполяции Тогда
...
Процедура СохранитьОбратныйСлешПриНеобходимости(Токен)

	Если Не ОткрытаКавычкаСПоддержкойИнтерполяции
		Или Не ЭкранироватьСимвол
		Или ОтменитьЭкранированиеСимвола Тогда
		Возврат;
	КонецЕсли;

	Если Токен <> Неопределено И СимволЭкранируетсяВДвойныхКавычкахBash(Токен) Тогда
		Возврат;
	КонецЕсли;
...
Функция СимволЭкранируетсяВДвойныхКавычкахBash(Токен)
...
Процедура ДобавитьПеременную(Знач Значение)
	...
	ШаблонВТексте = ОбщийНаборИнструментов.ПрефиксХешаПеременной() + "%1";

	Имя = Сред(Значение, 2);
	Имя = СтрЗаменить(Имя, "{", "");
	Имя = СтрЗаменить(Имя, "}", "");

	Если Переменные.Найти(Имя) = Неопределено Тогда
