...
Процедура РазобратьURL(Знач URL)
	...
	Если ЗначениеЗаполнено(АутентификацияСтрока) Тогда
		...
		ИндексДвоеточия = СтрНайти(АутентификацияСтрока, ":");
		Если ИндексДвоеточия > 0 Тогда
			Аутентификация.Пользователь = Лев(АутентификацияСтрока, ИндексДвоеточия - 1);
			Аутентификация.Пароль = Сред(АутентификацияСтрока, ИндексДвоеточия + 1);
		Иначе
			Аутентификация.Пользователь = АутентификацияСтрока;
		КонецЕсли;
...
Процедура ЗаполнитьПараметрыЗапроса(Путь)
	...
	Для Каждого СтрокаКлючРавноПараметр Из СтрРазделить(Запрос, "&", Ложь) Цикл
		...
		Если ОбработанныеКлючи.Получить(Ключ) = Истина Тогда
		...
		Иначе
			...
			ОбработанныеКлючи.Вставить(Ключ, Истина);
