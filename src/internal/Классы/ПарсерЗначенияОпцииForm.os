...
//       ** ФайлыЗаголовков - Массив из Строка - Пути к файлам с заголовками
...
Процедура ПроверитьИДобавитьПараметр()
	...
	Иначе
		...
		Если ИмяПараметра = "headers" Тогда
			ЗначениеЗаголовков = СокрЛП(ЗначениеПараметра);
			Если Лев(ЗначениеЗаголовков, 1) = "@" Тогда
				Результат.Значения[Результат.Значения.ВГраница()].ФайлыЗаголовков.Добавить(Сред(ЗначениеЗаголовков, 2));
			Иначе
				ДобавитьЗаголовок(ПараметрыЗначения["headers"], ЗначениеЗаголовков);
			КонецЕсли;
		Иначе
