...
Процедура ОтдатьФайл(Запрос, Ответ) Экспорт
	...
	Иначе
		...
		Если ТребуетПроверкиАктуальности(Запрос.Путь) Тогда
			Ответ.Заголовки["Cache-Control"] = "no-cache";
		Иначе
...
Функция ТребуетПроверкиАктуальности(Знач Путь)

	Путь = НРег(Путь);

	Возврат СтрЗаканчиваетсяНа(Путь, ".css")
		Или СтрЗаканчиваетсяНа(Путь, ".js")
		Или СтрЗаканчиваетсяНа(Путь, ".json")
		Или СтрЗаканчиваетсяНа(Путь, ".html");
