...
Функция ПараметрВСтроку(Значение) Экспорт
	...
	Если Тип = Тип("Строка") Тогда
		...
		Если Не СтрНачинаетсяС(Значение, "{t(Переменная.") Тогда
	...
	Для Каждого Значение Из Массив Цикл
		...
		Иначе
			...
			Если Значение = Неопределено Тогда
				ЗначениеВСтроку = "{t(Неопределено)}";
			Иначе
				ЗначениеВСтроку = ПараметрВСтроку(Значение);
			КонецЕсли;

			ДобавитьСтроку("%1.{t(Массив.Добавить)}(%2);", ИмяПараметра, ЗначениеВСтроку);
