...
Функция ДанныеЗапроса(НазначениеДанных, ТекстовыеДанные, ПрочитанныеФайлы) Экспорт
...
Функция URL(СтруктураURL, ТекстовыеДанные = Неопределено, ПрочитанныеФайлы = Неопределено) Экспорт
	
	// Схема
	URL = """" + СтруктураURL.Схема + "://";
	...
	// Сервер
	URL = URL + СтруктураURL.Сервер;

	// Порт
	Если ЗначениеЗаполнено(СтруктураURL.Порт) Тогда
		URL = URL + ":" + Формат(СтруктураURL.Порт, "ЧГ=");
	КонецЕсли;

	// Адрес ресурса
	АдресРесурса = АдресРесурса(СтруктураURL, ТекстовыеДанные, ПрочитанныеФайлы);

	Если ПустаяСтрока(АдресРесурса) Тогда
		URL = URL + """";
	Иначе
		URL = URL + Сред(АдресРесурса, 2);
	КонецЕсли;
...
Функция АдресРесурса(СтруктураURL, ТекстовыеДанные = Неопределено, ПрочитанныеФайлы = Неопределено) Экспорт
...
Функция ИнтернетПрокси(ОписаниеЗапроса, ИмяПараметра) Экспорт
