...
- единое конвейерное ядро лексического разбора, синтаксического разбора, интерпретации и генерации;
- публичные контракты результата, параметров и предупреждений конвертации;
- версионируемый SPI и реестр сторонних генераторов;
- цели `1c` и `connector` на общем семантическом контексте;
- HTTP API `POST /api/v2/convert` и OpenAPI 3.1;
- [матрица возможностей](docs/МатрицаВозможностейCURL.md) и
  [руководство миграции](docs/МиграцияНаCurlone2.md).

### Изменено

- библиотека, CLI и HTTP API используют одно ядро v2;
- CLI выполняет преобразование через `curlone convert` и поддерживает текстовый и JSON-форматы;
- ошибки создаются через `errors`, а потери семантики возвращаются отдельными предупреждениями;
- `resilience` обновлён до 0.4.0, переполнение ограничения параллелизма возвращает HTTP 503;
- каталог опций приведен к curl 8.16.0;
- минимальная версия OneScript повышена до 2.0.0.

### Удалено

- библиотечный API, CLI и HTTP API версии 1;
- прежние парсеры и генераторы из `src/internal`;
- пул конвертеров и неиспользуемые зависимости старой архитектуры.

### Известные ограничения

- Connector не поддерживает FTP и FTPS;
- часть настроек прокси, повторов, таймаутов и внешних источников представляется приближенно;
- alpha-версия не гарантирует стабильность SPI до окончательного выпуска 2.0.0.
...
[2.0.0-alpha.1]: https://github.com/alei1180/curlone/compare/ver1.8.0...ver2.0.0-alpha.1
