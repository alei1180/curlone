...
Процедура ТестДолжен_ОтображатьСсылкуНаВерсиюИзPackagedef() Экспорт
	...
	// Проверка
	Ожидаем.Что(ТекстСтраницы).Содержит("ver " + ВерсияПриложения);
	Ожидаем.Что(ТекстСтраницы).Содержит(ОжидаемаяСсылкаНаВерсию);
...
Процедура ТестДолжен_ВыполнитьКонвертацию() Экспорт
	...
	Для Каждого ТестовыйСлучай Из ТестовыеСлучаи Цикл
		...
		Json = СериализаторJSON.Сериализовать(ПараметрыЗапроса);
