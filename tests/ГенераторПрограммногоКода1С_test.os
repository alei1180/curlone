...
Функция ПолучитьСписокТестов(Знач Тестирование) Экспорт
	...
	СписокТестов.Добавить("ТестДолжен_ПроверитьИгнорированиеАктивногоРежимаПриИспользованииПроксиFTP");
...
Процедура ТестДолжен_ПроверитьПередачуФайла() Экспорт
	...
	КонсольнаяКоманда = "curl --upload-file path/to/file http://example.com ";
...
Процедура ТестДолжен_ПроверитьПередачуНесколькихФайловПоОдномуURL() Экспорт
	...
	КонсольнаяКоманда = "curl --upload-file {path/to/file1,path/to/file2} http://example.com";

	ПрограммныйКод = "Соединение = Новый HTTPСоединение(""example.com"", 80);
	|HTTPЗапрос = Новый HTTPЗапрос(""/"");
	|
	|// Передача файла 1. path/to/file1
	|HTTPЗапрос.УстановитьИмяФайлаТела(""path/to/file1"");
	|
	|HTTPОтвет = Соединение.ВызватьHTTPМетод(""PUT"", HTTPЗапрос);
	|
	|// Передача файла 2. path/to/file2
	|HTTPЗапрос.УстановитьИмяФайлаТела(""path/to/file2"");
	|
	|HTTPОтвет = Соединение.ВызватьHTTPМетод(""PUT"", HTTPЗапрос);";
...
Процедура ТестДолжен_ПроверитьИгнорированиеПередаваемыхФайловПриПревышенииКоличестваURL() Экспорт
	...
	Ожидаем.Что(Ошибки[0].Текст).Равно("Файл 'path/to/file3' опции -T, --upload-file было проигнорировано");
	Ожидаем.Что(Ошибки[0].Критичная).ЭтоЛожь();
	Ожидаем.Что(Ошибки[1].Текст).Равно("Файл 'path/to/file4' опции -T, --upload-file было проигнорировано");
